...
> ⚠️ **The state machine advances by two routes at once, and this line used to say only one of them existed.** `renderPedidos()` ([js/pedidos.js](js/pedidos.js), 5 writes — one to `ofertas`, four to `pedidos`) expires stale offers, reopens pedidos with no live offers, and marks past-due agreements `expirado`. And `sincronizar_estados_pedidos()` does the same in **pg_cron every 15 minutes, in production and in pruebas since 2026-09-11** — so `20260810130000_sincronizar_estados_OPCIONAL.sql` **is applied**, contrary to what this file claimed until 2026-09-25 (production's cron dump lists `portgo-sincronizar-estados │ */15 * * * * │ activo=true`). The browser copies were kept on purpose: idempotent, identical rules, and while both exist a cron failure doesn't freeze anything. Taking them out of the render is a later decision — see hueco 4 in [docs/FLUJO-OPERATIVO.md](docs/FLUJO-OPERATIVO.md).
