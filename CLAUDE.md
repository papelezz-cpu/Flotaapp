...
>
> It compares **only the `FOR ROLE postgres` rows**, because those are what govern the objects our migrations create, and they are the only ones that can be equalized: in Supabase `postgres` is not a superuser and cannot set another platform role's default privileges. Comparing `supabase_admin`'s would leave the stamp `diverge` forever, and a lock that can never open is a lock people learn to ignore. To see all of them — which is what to do when something was created from the dashboard — run `bash supabase/ver-privilegios-por-omision.sh <proyecto>`.
>
> **This was a real, measured divergence, not a hypothetical:** on 2026-09-24 `pg_default_acl` was **empty** in `portgo-pruebas` and had six rows in production. Consequence, with the *same* migration applied to both: the five H-04 functions and the `vigencias_caducidad` view came out with `service_role` privileges in production and without them in pruebas. The migration never mentions `service_role` — the schema did. Same symptom R-07 recorded on 2026-09-18 with `reporte_kpis()`. Step 4b of `replicar-produccion-a-pruebas.sh` now sets those defaults, so new objects are born identical in both projects.
...
> ⚠️ **The state machine lives in pg_cron, and ONLY there since 2026-09-25.** `sincronizar_estados_pedidos()` runs every 15 minutes in production and in pruebas (since 2026-09-11) and applies **all five** rules: expire stale offers, reopen pedidos whose offers are all rejected, complete a half-closed agreement to `pendiente_acuerdo`, expire past-due agreements, and expire solicitudes whose load date arrived with no live offer. `20260810130000_sincronizar_estados_OPCIONAL.sql` **is applied** — this file claimed otherwise until 2026-09-25 (production's cron dump lists `portgo-sincronizar-estados │ */15 * * * * │ activo=true`).
>
> ⚠️ **The migration file has four of those rules; the LIVE function has five.** Rule (e), `solicitudes_vencidas`, was added to the browser afterwards and then to the function, never to the file. Verify this against `pg_get_functiondef`, never against the `.sql` — reading the file is what nearly shipped the render change with one rule uncovered.
>
...
**Closure evidence:** each side has its own column — `evidencias` (company) and `evidencias_cliente` (client), both arrays of storage **paths**; plus `finalizacion_solicitada_por`. **Whichever side uploads first moves the reservation `Activa` → `PorAprobar`** — `registrar_evidencias` does `estado = CASE WHEN v_solicitando THEN 'PorAprobar' ELSE estado END`, where `v_solicitando` is `estado = 'Activa'`. The SA can approve with only one side's evidence: `aprobarFinalizacion()` never checks for both, the panel just flags `⚠️ falta` next to the missing one. (This line used to say both sides were required, until the code said otherwise on 2026-09-11.)
...
`reservaciones_historico`: archived rows + `archivado_en` / `archivado_por`. **`archivado_en` was `archivado_at` until 2026-09-25** (H-22: everything else in the schema uses the `_en` suffix). The client does **not** send it — the column's `DEFAULT now()` fills it, so the value is the server's clock and not the browser's, and renaming it needed no deploy window. Nobody reads it: it isn't rendered, ordered by, or referenced in any policy or function.
